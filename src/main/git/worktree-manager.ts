...
/**
 * Fetch a branch from origin and report where it exists.
 *
 * `remote` is only probed when the branch is absent locally, so a local branch
 * that is ahead of its remote still reports `{ local: true, remote: false }`.
 * Every git call is best-effort: offline, no remote, or an unknown branch all
 * resolve to `{ local: false, remote: false }`.
 */
...
export class WorktreeManager {
  ...
  async createWorktree(
    ...
    if (customBranchName) {
      const location = await locateBranch(this.git, branchName);
      if (location.remote) {
        await this.git.raw(['branch', branchName, `origin/${branchName}`]);
      }
      branchExists = location.local || location.remote;
    }
